...
int main(int arcg, char** argv) {
    ...
    //uint64_t card = 232220291760329372127997141845981;
    //std::string hand = get_string_from_id(card);
    //std::cout << "Original hand: " << hand << std::endl;
    ...
    for (size_t idx=0; idx<keys.size(); ++idx) {
        ...
        if (idx % 1000000 == 0) {
            std::cout << "Processed " << 1000000 << " hands." << std::endl;
        }
    }
    ...
    for (const auto& pair : canonicals) {
        ...
        ++counter;
        if (counter % 100000 == 0) {
            std::cout << "Written " << 100000 << " rows." << std::endl;
        }
    }
    return 0;
}