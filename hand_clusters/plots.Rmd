...
plot_bins = function(bins){
  ...
  df = data.frame(
    x_min = head(bin_positions, -1),
    x_max = tail(bin_positions, -1),
    height = bin_heights
  )
  
  ggplot(df, aes(xmin = x_min, xmax = x_max, ymin = 0, ymax = height)) +
...
ob = data[data$V1 == "2C2D2H9HJS", ]
hist(as.numeric(ob), breaks=50)
```
